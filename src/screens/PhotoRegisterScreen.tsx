...
export default function PhotoRegisterScreen({ navigation }: Props) {
  ...
  const createDummyImage = () => {
    const dummyImage: ImagePickerAsset = {
    ...
    setPickedImage(dummyImage);
  };

  // 카메라 촬영 또는 시뮬레이터용 더미 이미지
  async function takeImageHandler() {
    // 시뮬레이터에서는 더미 이미지 사용
    if (isSimulator) {
      Alert.alert(
        '시뮬레이터 모드',
        '시뮬레이터에서는 더미 이미지를 사용합니다.',
        [
          { text: '취소', style: 'cancel' },
          { text: '확인', onPress: createDummyImage },
        ],
      );
      return;
    }
