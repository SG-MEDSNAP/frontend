...
export function TimePickField({ control }: { control: Control<MedicationForm> }) {
  ...
  const pickerItemStyle = { color: isDark ? '#fff' : '#111', fontSize: 20 as const, /* @ts-ignore */ fontFamily: 'System' };
          ...
          <>
            ...
            >
              <View className="flex-1 justify-end">
                ...
                >
                  <View className="flex-row gap-3 justify-center items-center" style={{ height: 216 }}>
                    <Picker
                    ...
                    >
                      {hours12.map((h) => <Picker.Item key={h} label={pad2(h)} value={h} />)}
                    </Picker>
                    <Picker
                    ...
                    >
                      {mins.map((mm) => <Picker.Item key={mm} label={pad2(mm)} value={mm} />)}
                    </Picker>
                  </View>

                  {dupMsg && (
                    <Text className="text-center text-[#FF5B6B] mt-2 text-[12px]">{dupMsg}</Text>
                  )}
